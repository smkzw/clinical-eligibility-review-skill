...
- OCR/VL API success is not evidence success. Treat repeated boilerplate, model self-reference, prompt-like text, header-only tables, and source text that fails the rule-specific semantic gate as OCR/VL failure or suspect output, even if the tool returned `success`.
- If a high-quality source page is expected to contain a critical eligibility fact but OCR/VL output misses it, do not mark the fact absent until a higher-confidence retry, suitable-DPI OCR, VLM fallback, Agent visual transcription, or human visual QC has been considered and recorded.
...
- When Agent/VLM/human visual review recovers a rule-critical fact that OCR missed, store it as structured derived evidence such as `subjects/<SUBJECT>/evidence/visual_evidence_items.jsonl` and merge it through the evidence layer. Do not patch the final HTML directly. Mark partial, obscured, or visually uncertain entries with `requires_source_verification` so they do not suppress EDC/email verification warnings.
...
- The visible decision basis must be clinical reasoning, not report operation text. Remove artifacts such as `仅EDC证据`, `仅二手来源`, `报告需默认展开`, `已找到候选证据`, `离线草稿`, `LLM/医学审核`, raw evidence IDs, and source-routing notes from the reasoning sentence. Source caveats belong in badges, priority warnings, and Excel source-consistency fields.
...
python -m eligibility_review.cli qc-report --project <PROJECT_A> --project <PROJECT_B> --expected-center <CENTER> --expected-subject-count <N> --expected-rule-row-count <N>
```
...
- Package rules must slice each component to its own short, rule-specific clause. If OCR uses ASCII semicolons or long comma-separated runs, treat them as practical boundaries instead of highlighting the whole paragraph. EX-07 disease-history package must not borrow EX-06 medication-history evidence. In particular, `免疫抑制/机会感染/频发感染` means immune-suppressed state or opportunistic/recurrent/long-term infection history, not `免疫抑制剂` medication use. If the source says `否认为免疫抑制者`, cite that disease-history clause.
...
- `subjects/<SUBJECT>/evidence/visual_evidence_items.jsonl`: optional structured derived visual-transcription evidence for OCR/VL misses; merge through evidence build, not HTML patches.
...
- When a project has expected center/subject/rule-row counts, run the executable full-batch QC command with those counts. A valid multi-center report should pass checks for HTML/Excel parity, internal-token leakage, IE/IEYN aggregate misuse, non-presence-only pass without displayable evidence, EX-08 FEV1 support/verification status, and package-rule adjacent-clause borrowing.
